...
import { main } from "./smoke.ts";
import { getPinchtabConfig } from "./pinchtab-api.ts";
...
test("config loads token from the PinchTab config candidates", async () => {
  const config = await getPinchtabConfig({ HOME: "/operator" }, {
...
test("smoke reuses a running profile and converts gif to webm", async () => {
  ...
  const fetch = async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (href.endsWith("/record/stop")) {
      return response({
        path: "/tmp/profile/.pinchtab-state/recordings/rec.gif",
      });
    }
